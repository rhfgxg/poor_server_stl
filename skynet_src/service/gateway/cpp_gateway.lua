--[[
    ...
    职责：
    1. 接收来自 C++ Gateway 的 TCP 连接（被动模式，C++ -> Skynet）
    2. 提供接口给 Skynet 服务向 C++ 发送请求（主动模式，Skynet -> C++）
    3. 双向复用同一条 TCP 连接
    ...
    ┌─────────────────────────────────────────────────────────┐
    │                   Skynet 集群                           │
    │ player_agent / battle / matching / ...                  │
    │        │                                                │
    │        │ skynet.call(gateway, "lua", "request", ...)    │
    │        ▼                                                │
    │ ┌──────────────────────────────────────┐                │
    │ │        cpp_gateway (本服务)           │                │
    │ │ - 监听端口接收 C++ 请求               │                │
    ...
    return encode_response(rpc.MSG.GET_PLAYER_STATUS_RES, {
        ...
        player_data = {
            ...
            local function process_inbound_message(fd, msg_type, msg_data)
                ...
                -- Echo 消息直接使用原始字符串
                if msg_type == rpc.MSG.ECHO then
                    local ok, result = pcall(handler, conn, msg_data)
                    if ok then
