...
int main()
{
    try
    {
        ...
        logger_manager.initialize(rpc_server::ServerType::LOGIN);

        logger_manager.getLogger(poor::LogCategory::STARTUP_SHUTDOWN)->info("Login_server starting...");
...
void run_server(LoggerManager& logger_manager)
{
    ...
    LoginServerImpl login_server(logger_manager, address, port);
    ...
    builder.RegisterService(&login_server);
 ...
 * 
 * 支持多种配置文件路径（按优先级）：
 * 1. config/cfg_login_server.lua (运行时目录的 config/)
 * 2. config/cpp/cfg_server/cfg_login_server.lua (项目根目录)
 * 3. ../config/cpp/cfg_server/cfg_login_server.lua (从 build/ 运行)
 */
void read_server_config(std::string& address, std::string& port)
{
    // 可能的配置文件路径（按优先级排序）
    const char* possible_paths[] = {
        "config/cfg_login_server.lua",                          // build/src/login/config/ (copy_config.sh 复制后)
        "config/cpp/cfg_server/cfg_login_server.lua",           // 从项目根目录运行
        "../config/cpp/cfg_server/cfg_login_server.lua",        // 从 build/ 目录运行
        "../../config/cpp/cfg_server/cfg_login_server.lua",     // 从 build/src/login/ 运行（但没有 config/）
        "../../../config/cpp/cfg_server/cfg_login_server.lua"  // 备用路径
    };
