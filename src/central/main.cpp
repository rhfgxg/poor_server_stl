 ...
 * 
 * 支持多种配置文件路径（按优先级）：
 * 1. config/cfg_central_server.lua (运行时目录的 config/)
 * 2. config/cpp/cfg_server/cfg_central_server.lua (项目根目录)
 * 3. ../config/cpp/cfg_server/cfg_central_server.lua (从 build/ 运行)
 */
void read_server_config(std::string& address, std::string& port)
{
    // 可能的配置文件路径（按优先级排序）
    const char* possible_paths[] = {
        "config/cfg_central_server.lua",                          // build/src/central/config/ (copy_config.sh 复制后)
        "config/cpp/cfg_server/cfg_central_server.lua",           // 从项目根目录运行
        "../config/cpp/cfg_server/cfg_central_server.lua",        // 从 build/ 目录运行
        "../../config/cpp/cfg_server/cfg_central_server.lua",     // 从 build/src/central/ 运行（但没有 config/）
