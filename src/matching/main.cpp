#include "./server/matching_server.h"
...
int main()
{
    try
    {
        ...
        logger_manager.initialize(rpc_server::ServerType::MATCHING);

        logger_manager.getLogger(poor::LogCategory::STARTUP_SHUTDOWN)->info("Matching_server starting...");
...
void run_server(LoggerManager& logger_manager)
{
    ...
    std::string port = "50057";
    read_server_config(address, port);

    // 创建服务器实例（BaseServer 会自动管理线程池、注册、心跳）
    MatchingServerImpl matching_server(logger_manager, address, port);

    // 启动服务器（BaseServer 自动处理初始化）
    if (!matching_server.start())
    {
    ...
    builder.RegisterService(&matching_server);
 ...
 * 
 * 支持多种配置文件路径（按优先级）：
 * 1. config/cfg_matching_server.lua (运行时目录的 config/)
 * 2. config/cpp/cfg_server/cfg_matching_server.lua (项目根目录)
 * 3. ../config/cpp/cfg_server/cfg_matching_server.lua (从 build/ 运行)
 */
void read_server_config(std::string& address, std::string& port)
{
    // 可能的配置文件路径（按优先级排序）
    const char* possible_paths[] = {
        "config/cfg_matching_server.lua",                          // build/src/matching/config/
        "config/cpp/cfg_server/cfg_matching_server.lua",           // 从项目根目录运行
        "../config/cpp/cfg_server/cfg_matching_server.lua",        // 从 build/ 目录运行
        "../../config/cpp/cfg_server/cfg_matching_server.lua",     // 从 build/src/matching/ 运行
        "../../../config/cpp/cfg_server/cfg_matching_server.lua"  // 备用路径
    };
