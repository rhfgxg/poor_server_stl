...
int main()
{
    ...
    logger_manager.initialize(rpc_server::ServerType::MATCHING);    // 传入服务器类型，创建日志文件夹

    // 记录启动日志
    logger_manager.getLogger(poor::LogCategory::STARTUP_SHUTDOWN)->info("Matching server started"); // 记录启动日志：日志分类, 日志内容
...
void RunServer(LoggerManager& logger_manager)
{
    ...
    builder.RegisterService(&matching_server); // 注册服务
    ...
    matching_server.start_thread_pool(4); // 启动4个线程处理请求

    server->Wait(); // 等待服务器终止
    matching_server.stop_thread_pool(); // 停止线程池
}
...
void Read_server_config(std::string& address, std::string& port)
{
    try
    {
        ...
        std::string file_url = "config/cfg_matching_server.lua";  // 配置文件路径
