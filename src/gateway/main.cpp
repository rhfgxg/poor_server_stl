...
int main()
{
    ...
    // 记录启动日志
    logger_manager.getLogger(poor::LogCategory::STARTUP_SHUTDOWN)->info("Gateway_server started"); // 记录启动日志：日志分类, 日志内容
...
void Read_server_config(std::string& address, std::string& port)
{
    ...
    std::string file_url = "config/cfg_gateway_server.lua";  // 配置文件路径
