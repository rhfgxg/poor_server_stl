...
int main()
{
    try
    {
        ...
        logger_manager.initialize(rpc_server::ServerType::GATEWAY);

        logger_manager.getLogger(poor::LogCategory::STARTUP_SHUTDOWN)->info("Gateway_server starting...");
...
void run_server(LoggerManager& logger_manager)
{
    ...
    GatewayServerImpl gateway_server(logger_manager, address, port);

    // 启动服务器（BaseServer 自动处理初始化）
    if (!gateway_server.start())
    {
    ...
    builder.RegisterService(&gateway_server);
 ...
 *
 * 支持多种配置文件路径（按优先级）：
 * 1. config/cfg_gateway_server.lua (运行时目录的 config/)
 * 2. config/cpp/cfg_server/cfg_gateway_server.lua (项目根目录)
 * 3. ../config/cpp/cfg_server/cfg_gateway_server.lua (从 build/ 运行)
 */
void read_server_config(std::string& address, std::string& port)
{
    // 可能的配置文件路径（按优先级排序）
    const char* possible_paths[] = {
        "config/cfg_gateway_server.lua",                          // build/src/gateway/config/ (copy_config.sh 复制后)
        "config/cpp/cfg_server/cfg_gateway_server.lua",           // 从项目根目录运行
        "../config/cpp/cfg_server/cfg_gateway_server.lua",        // 从 build/ 目录运行
        "../../config/cpp/cfg_server/cfg_gateway_server.lua",     // 从 build/src/gateway/ 运行（但没有 config/）
        "../../../config/cpp/cfg_server/cfg_gateway_server.lua"  // 备用路径
    };
