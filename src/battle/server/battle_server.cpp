...
BattleServerImpl::BattleServerImpl(LoggerManager& logger_manager_, const std::string address, std::string port):
    server_type(rpc_server::ServerType::BATTLE),
    ...
    logic_connection_pool(10)
{
    ...
    // 启动定时任务，
    // 定时向中心服务器获取最新的连接池状态
    std::thread(&BattleServerImpl::Update_connection_pool, this).detach();
    // 定时向中心服务器发送心跳包
    std::thread(&BattleServerImpl::Send_heartbeat, this).detach();
}

BattleServerImpl::~BattleServerImpl()
{
...
/************************************ 线程池工具函数 ****************************************************/
// 启动线程池
void BattleServerImpl::start_thread_pool(int num_threads)
{// 相关注释请参考 /src/central/src/central/central_server.cpp/start_thread_pool()
    for(int i = 0; i < num_threads; ++i)
    {
        this->thread_pool.emplace_back(&BattleServerImpl::Worker_thread, this);   // 创建线程
    }
}

// 停止线程池
void BattleServerImpl::stop_thread_pool()
...
// 添加异步任务
std::future<void> BattleServerImpl::add_async_task(std::function<void()> task)
{
...
// 线程池工作函数
void BattleServerImpl::Worker_thread()
...
/************************************ 连接池管理 ********************************************************/
// 注册服务器
void BattleServerImpl::register_server()
{
    // 请求
...
// 注销服务器
void BattleServerImpl::unregister_server()
{
    // 请求
...
// 初始化连接池
void BattleServerImpl::Init_connection_pool()
{
    // 客户端
    grpc::ClientContext context;
    // 请求
...
/************************************ 定时任务 ********************************************************/
// 定时任务：更新连接池
void BattleServerImpl::Update_connection_pool()
{
    while(true)
    {
...
// 定时任务：发送心跳包
void BattleServerImpl::Send_heartbeat()
{
    while(true)
    {
